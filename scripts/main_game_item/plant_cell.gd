...
func create_plant(plant_type:CharacterRegistry.PlantType, is_imitater:=false, is_plant_start_effect:=true, is_imitater_material:=false, is_zombie_mode:=false) -> Plant000Base:
	...
	if plant_condition.is_purple_card:
		...
		if is_instance_valid(plant_in_cell[condition_pre_plant.place_plant_in_cell]):
			## 标记为升级替换死亡，防止 Host 重复广播 plant_death 导致客户端误杀新植物
			plant_in_cell[condition_pre_plant.place_plant_in_cell].is_upgrade_replaced = true
...
## 是否有植物（联机铲除验证用）
func has_plant() -> bool:
	return get_curr_plant_num() > 0

## 铲除当前格子的植物（联机用，默认铲除 Norm 位置植物）
func shovel_plant() -> void:
	var plant = return_plant_be_shovel_look()
	if plant:
