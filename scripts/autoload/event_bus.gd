...
const NETWORKED_EVENTS := [
	...
	"zombie_go_home",
	"create_trophy",
	"main_game_progress_update",
]

## 推送网络事件（Host 端调用，自动广播给所有客户端）
func push_networked_event(event_name: String, payload: Variant = []) -> void:
	if not payload is Array:
		payload = [payload]
	# 本地触发
	push_event(event_name, payload)
	# 如果是多人模式且是服务器，广播给客户端
	if NetworkManager.is_multiplayer and multiplayer.is_server():
		_remote_push_event.rpc(event_name, payload)

## Host → 客户端: 远程推送事件
@rpc("authority", "reliable")
func _remote_push_event(event_name: String, payload: Array) -> void:
