...
func set_game_para() -> void:
	...
	if is_multiplayer:
		multiplayer_player_count = NetworkManager.player_count
...
func on_zombie_go_home(zombie:Zombie000Base):
	## 多人模式：仅 Host 处理失败逻辑并广播
	if is_multiplayer and not NetworkManager.is_server():
		return
	if is_multiplayer:
		## broadcast_game_lose 有 call_local, 会在 Host 和 Client 都执行 _on_game_lose_from_network
		NetworkManager.broadcast_game_lose.rpc()
		## Host 额外处理僵尸进家动画（客户端没有对应僵尸引用）
...
#region 奖杯
## 创建奖杯（由 zombie_manager 触发，仅 Host 执行）
func create_trophy(glo_pos:Vector2):
	## 多人模式：仅 Host 判定胜利并广播
	if is_multiplayer and not NetworkManager.is_server():
		return
	if is_multiplayer:
		## broadcast_game_win 有 call_local，会在所有端（含 Host）调用 _do_create_trophy
		NetworkManager.broadcast_game_win.rpc(glo_pos.x, glo_pos.y)
		return
	_do_create_trophy(glo_pos)

## 实际创建奖杯（Host 和 Client 共用，由 broadcast_game_win RPC 或单人模式调用）
func _do_create_trophy(glo_pos: Vector2) -> void:
	print("胜利条件达成，创建奖杯")
	## 如果不是最后一轮游戏，触发下一轮
	if curr_game_round != game_para.game_round:
		start_next_round_game()
...
## 客户端收到 Host 广播的失败消息
func _on_game_lose_from_network() -> void:
	re_main_game()
	main_game_progress = E_MainGameProgress.GAME_OVER
	card_slot_root.visible = false
	camera_2d.process_mode = Node.PROCESS_MODE_ALWAYS
