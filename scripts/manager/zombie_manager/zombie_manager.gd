...
var curr_zombie_num:int = 0:
	set(v):
		...
		label_zombie_sum.visible=false
...
## 多人模式：网络 ID 映射表 {network_id: Zombie000Base}
var _zombie_by_net_id: Dictionary = {}
## 多人模式：递增的网络 ID 计数器
var _next_zombie_net_id: int = 1
...
func create_norm_zombie(
	...
	if NetworkManager.is_multiplayer:
		if NetworkManager.is_server():
			zombie.network_id = _next_zombie_net_id
			_next_zombie_net_id += 1
		_zombie_by_net_id[zombie.network_id] = zombie
...
func _on_zombie_hypno(zombie:Zombie000Base):
	...
	if is_end_wave and curr_zombie_num == 0:
		## 多人模式下仅 Host 触发胜利
		if not main_game.is_multiplayer or NetworkManager.is_server():
...
func _on_zombie_dead(zombie: Zombie000Base) -> void:
	...
	if zombie.network_id >= 0:
		_zombie_by_net_id.erase(zombie.network_id)
	if zombie.is_hypno:
