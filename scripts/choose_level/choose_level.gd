...
func _on_choose_level_button(choose_level_button:ChooseLevelButton):
	...
	## 多人模式：仅 Host 可选关，广播给所有客户端
	if NetworkManager.is_multiplayer:
		if not NetworkManager.is_server():
			return  # 客户端不能选关
		var res_path = choose_level_button.curr_level_data_game_para.resource_path
		var scene_key = int(choose_level_button.curr_level_data_game_para.game_sences)
		NetworkManager.broadcast_level_chosen.rpc(res_path, scene_key)
		return
